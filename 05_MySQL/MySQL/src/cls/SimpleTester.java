package cls;
import Join.Question10;
import ScalarFunctions.Question13;
...
public class SimpleTester {
    public static void main(String[] args) throws SQLException {
        ...
        // SCALAR FUNCTIONS
        // ScalarFunc_Question13();

        // JOIN
        Join_Question10();
    }

    private static void Join_Question10() throws SQLException {
        List<Question10> myList = new ArrayList<>();
        Map<Integer,Object> params = new HashMap<>();
        ResultSet results = DButils.runQueryFroResult(SQLcommands.question10,params);

        while (results.next()){
            int productID = results.getInt(1);
            String categoryDescription = results.getString(2);
            String country = results.getString(3);
            myList.add(new Question10(productID,categoryDescription,country));
        }

        myList.forEach(System.out::println);
    }

    private static void ScalarFunc_Question13() throws SQLException {
        List<Question13> myList = new ArrayList<>();
        Map<Integer,Object> params = new HashMap<>();
        ResultSet results = DButils.runQueryFroResult(SQLcommands.question13,params);

        while (results.next()){
            String product = results.getString(1);
            int fullPrice = results.getInt(2);
            myList.add(new Question13(product,fullPrice));
        }

        myList.forEach(System.out::println);
    }

    private static void getDataFromSQL(List<Object> myList) throws SQLException {
